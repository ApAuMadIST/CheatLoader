		...
		for(int i = 0; i < localaz.getSizeInventory(); ++i) {
			...
			if(localiz != null) {
				...
				while(localiz.stackSize > 0) {
					...
					localiz.stackSize -= j;
					EntityItem localhl = new EntityItem(paramfd, (double)((float)paramInt1 + f1), (double)((float)paramInt2 + f2), (double)((float)paramInt3 + f3), new ItemStack(localiz.itemID, j, localiz.getItemDamage()));
					float f4 = 0.05F;
